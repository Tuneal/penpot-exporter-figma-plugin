...
const componentsMap: { [name: string]: any } = {};

export default class PenpotExporter extends React.Component<
  PenpotExporterProps,
  PenpotExporterState
> {
  ...
  createPenpotShapesFromComponent(file, node, baseX, baseY) {
    componentsMap[node.name] = node;
    file.addGroup({ name: node.name });
  ...
  createPenpotShapesFromComponentInstance(file, node, baseX, baseY) {
    const componentNode = componentsMap[node.name];
    if (componentNode) {
      this.createPenpotShapesFromComponent(
        file,
        {
          ...componentNode,
          x: node.x,
            ...
            children: [
              {
                ...
                textAlign: this.translateHorizontalAlign(
  ...
  createPenpotItem(file, node, baseX, baseY) {
    ...
    if (hasImageFill) {
      ...
      this.createPenpotShapesFromComponent(file, node, baseX, baseY);
    } else if (node.type == "INSTANCE") {
      this.createPenpotShapesFromComponentInstance(file, node, baseX, baseY);
    ...
    return (
      <main>
        ...
        <section>
          <div
            style={{
              display: this.state.missingFonts.size > 0 ? "inline" : "none",
            }}
          >
